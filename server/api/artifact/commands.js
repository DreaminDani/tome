...
const addComment = async (req, res) => {
  ...
  try {
    ...
    let insertComment;
    if (commentsLength > 0) {
      // if UUID is passed, use it instead of the autogenerated one
...
const updateComment = async (req, res) => {
  ...
  try {
    ...
    const insertComment = await client.query(
      `UPDATE artifacts
        SET artifact_data = jsonb_insert(artifact_data, '{comments,${commentsLength}}', ('{"id": "' || $1 || '", "user": {"id": '|| $2 ||', "name": "'|| $3 ||'"}, "comment": '|| $4 ||'}')::jsonb, true)
        WHERE id = $5
        RETURNING artifact_data->'comments' as commentlist
`,
      [
